...
/**
 * 原生Properties调用store方法保存时，
 * 会把文件中原来的注释覆盖，而且顺序会发生错乱，
 * 
 * 重写java.util.Properties文件读写类，
 * 在读取properties文件的时候把注释和格式按照记录，
 * 操作时候也记录添加顺序
 *
 */
...
public class SafeProperties extends Properties {
    ...
    public synchronized void store(Writer writer , String header) throws IOException {
    	BufferedWriter awriter = (writer instanceof BufferedWriter) ? (BufferedWriter)writer : new BufferedWriter(writer); 
        List entrys = context.getCommentOrEntrys();
        for (Iterator iter = entrys.iterator(); iter.hasNext();) {
            Object obj = iter.next();
            if (obj.toString() != null) {
        ...
        class PropertyEntry {
            ...
            public String toString() {
                ...
                if (key != null && value != null) {
                    ...
                    String v = saveConvert(value, false, false);
                    return k + "=" + v;
                }
                return null;
            }
