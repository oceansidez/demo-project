...
@Component
@Resource(name = "redisUtil")//根据name注入，相当于是放在set的方法上  ？？？
public class RedisUtil {

	@Autowired
	@Qualifier("shardedJedisPool")//根据名称进注入
	...
	/**
	 * 向redis存入key和序列化的object,并释放连接资源 如果key已经存在 则覆盖
	 * 
	 * @param key
	 * @param object
	 * @return 成功 返回OK 失败返回 0
	...
	/**
	 * 向redis存入key和序列化的object,并释放连接资源,指定过期时间 如果key已经存在 则覆盖
	 * 
	 * @param key
	 * @param object
	 * @return 成功 返回OK 失败返回 0
	...
	/**
	 * 通过字节数组key获取储存在redis中的反序列化的Object 并释放连接
	 * 
	 * @param key 字节数组
	 * @return 成功返回value 失败返回null
	 */
	 ...
	 * @return
	 */
	public String setoj(String key, Object object) {
		ShardedJedis jedis = null;
		String objectJson = null;
		try {
			jedis = shardedJedisPool.getResource();
			objectJson = JsonUtil.toJson(object);
	...
	/**
	 * 向redis存入key和json格式,并释放连接资源,指定过期时间 如果key已经存在 则覆盖
	 * 
	 * @param key
	 * @param object
	 * @return 成功 返回OK 失败返回 0
	 */
	public String setojex(String key, Object object, int seconds) {
		ShardedJedis jedis = null;
		String objectJson = null;
		try {
			jedis = shardedJedisPool.getResource();
			objectJson = JsonUtil.toJson(object);
			return jedis.setex(key, seconds, objectJson);
		} catch (Exception e) {
	 ...
	 * @return
	 */
	public <T> T getoj(String key, Class<T> clazz) {
		ShardedJedis jedis = null;
		String value = null;
		try {
			jedis = shardedJedisPool.getResource();
	 ...
	 * @param redis
	 */
	public static void closeResource(ShardedJedis jedis) {
		if (jedis != null) {
			jedis.close();
		}
	}
