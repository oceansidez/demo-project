...
import com.telecom.pay.wechat.pc.*;
 ...
 * 
 */
public class WechatPayPC {
	
	private static final Log log = LogFactory.getLog(WechatPayPC.class);
	...
	public static String pay(PayBean bean, String appId, String partner, String partnerKey,
		...
		try {
			...
			paraMap.put("spbill_create_ip", bean.getServerIp());
			...
			buf.append("<spbill_create_ip>" + bean.getServerIp() + "</spbill_create_ip>");
			buf.append("<notify_url>" + notify_url + "</notify_url>");
			buf.append("<trade_type>" + "NATIVE" + "</trade_type>");
			...
			for (int i = 0; i < list.size(); i++) {
				...
				} else if (map.get("key") != null && StringUtils.equals(map.get("key").toString(), "code_url")) {
					if (map.get("value") != null) {
						code_url = map.get("value").toString();
					}
			...
			paramMap.put("code_url", code_url);
			...
			if (StringUtils.equals(hmac, sign)) {
				...
				rtnMap.put("code_url", code_url);
				
				Double format = Double.parseDouble(bean.getFee());
				DecimalFormat df = new DecimalFormat("0.00");
				String formatFee = df.format(format);
				bean.setFee(formatFee);
				
				result = BuildHtmlUtil.buildCodeHtml(code_url, bean.getReturnUrl(),
						bean.getTradeNo(), bean.getName(), formatFee, projectName, path);
				return result;
			} else {
				log.info("【微信支付】微信下单异常，签名不符");

				result = BuildHtmlUtil.buildErrorHtml("微信支付签名不符", projectName, path);
				return result;
			}
		} catch (Exception e) {
			if (reader != null) {
			...
			try {
				result = BuildHtmlUtil.buildErrorHtml("微信支付发生异常！请联系系统管理员", projectName, path);
			} catch (Exception e1) {
				e1.printStackTrace();
			}
			return result;
	...
	public static PayResult notifyResult(HttpServletRequest request, String appId, String appSecret, String partner, String partnerKey) {
		try {
			...
			if (validate(partnerKey, sb.toString())) {
				...
				log.info("微信订单查询接口结果【trade_state】: "
						+ map.get("trade_state").toString());
				if (map.get("trade_state") != null
						&& map.get("trade_state").toString().equals("SUCCESS")) {
				...
				payResult.setPayTime(DateOperateUtil.stringToDateWithFormat(
						time_end, "yyyyMMddHHmmss"));
				payResult.setPayer(openid);

				// 处理同步跳转页面
				PayWebSocket pws = GlobalValue.payWebSocketSet.get
						(CommonConstant.WebSocketPerfix.PAY + payResult.getTradeNo());
				...
				pws.sendMessage(param);
				return payResult;
			} else {
				PayResult result = new PayResult();
				result.setPayFlag(false);
				result.setDesc("微信PC端Validate验证未通过");
				return result;
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static boolean validate(String partnerKey, String strReader){
		try {
			...
			System.out.println("====================");
			System.out.println("微信支付PC端validate验证:");
