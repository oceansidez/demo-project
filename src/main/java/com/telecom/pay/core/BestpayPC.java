...
import com.telecom.pay.bestpay.pc.BestpaySubmit;
import com.telecom.pay.bestpay.pc.CryptTool;
import com.telecom.pay.bestpay.pc.OtherTool;
...
public class BestpayPC {
	...
	public static String pay(PayBean bean, String merchantId, String key) {
		...
		String PRODUCTAMOUNT = OtherTool.changeY2F(bean.getFee());//产品金额(分)
		String ATTACHAMOUNT = "0";//附加金额(分)
		String ORDERAMOUNT = OtherTool.changeY2F(bean.getFee());//订单总金额(分)
		...
		String sourceStr="MERCHANTID="+MERCHANTID+"&ORDERSEQ="+ORDERSEQ+"&ORDERDATE="+ORDERDATE+"&ORDERAMOUNT="+ORDERAMOUNT+"&CLIENTIP="+CLIENTIP+"&KEY="+commkey;
		String mac = "";
		try {
			mac = CryptTool.md5Digest(sourceStr);
		} catch (Exception e) {
			System.out.println("============翼支付PC端支付生成签名异常===========");
			e.printStackTrace();
		}
	...
	public static PayResult notifyResult(HttpServletRequest request, String key) {
		...
		StringBuffer logInfoStringBuffer = new StringBuffer("PC端翼支付异步通知: ");
		...
		try {
			...
			if (sign != null && sign.length() > 0) {
				if(sign.equals(checkMac)){
					...
					PayResult result = new PayResult();
					result.setPayFlag(false);
					result.setDesc("翼支付PC端Validate验证未通过");
					return result;
				}
			}
			else {
